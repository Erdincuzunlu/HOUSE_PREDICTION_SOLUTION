...
test = pd.read_csv("/Users/erdinc/PycharmProjects/pythonProject3/Machine_Learning/datasets/test.csv")
train = pd.read_csv("/Users/erdinc/PycharmProjects/pythonProject3/Machine_Learning/datasets/train.csv")
...
# Histograma baktığımızda bağımlı değişkenin sola çarpık olduğunu görüyoruz .
# Bu verinin dağılımını daha simetrik bir hale getirmek ve regresyon modelini daha doğru hale getirmek için bazı
# dönüşümler yapmamız gerektiğini gösteriyor. Örnek olarak ;
# log dönüşümü yapabiliriz.

np.log1p(df["SalePrice"]).hist(bins=20)
plt.show()

# Evet log dönüşümü sonrası veri seti biraz daha simetrik bir hal aldı ve normal dağılıma yaklaştı.
...
# Yüksek korelasyona sahip değişkenleri belirlemek gereksiz değişkenleri veri setinden çıkarmak için yardımcı olan fonksiyon
def high_correlated_cols(dataframe, plot=False, corr_th=0.70):
    # Sadece sayısal sütunları seç
    num_cols = dataframe.select_dtypes(include=[np.number]).columns

    corr = dataframe[num_cols].corr()  # Sadece sayısal sütunların korelasyon matrisi
    cor_matrix = corr.abs()
...
# Numerik değişkenlerin aykırı değerlerinin baskılanması
for col in num_cols:
    if col != "SalePrice":
        replace_with_thresholds(df, col)

# Baskılama sonrası  sütunları kontrol et!
...
# Eksik değerlerin Median ya da mean ile doldurulması
...
print(df.dtypes)
# Tüm kategorik sütunları sayısala dönüştürme
for col in df.columns:
    if df[col].dtype == 'O':
        df[col] = df[col].astype('category').cat.codes

# TotalQual hesaplama (tüm kategorik sütunlar sayısala dönüştürüldü)
...
#
# Test verisi üzerinde tahminler yapıldı.

y_pred # değerler geldi tabloda.
...
plot_importance(model, X, num=40)
